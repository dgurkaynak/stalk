...
export default class MouseHandler extends EventEmitterExtra {
  private machine = Machine<MouseHandlerStateContext, MouseHandlerStateSchema, MouseHandlerStateEvent>({
    ...
    guards: {
      isDifferentButton: (context, event, { cond }) => {
        const refMouseEvent: MouseEvent | null = (context as any)[(cond as any).with];
      ...
      isTooLateForDoubleClick: (context, event, { cond }) => {
        const refMouseEvent: MouseEvent | null = (context as any)[(cond as any).with];
        if (!refMouseEvent) return false;
        return event.mouseEvent.timeStamp - refMouseEvent.timeStamp >= DOUBLE_CLICK_WAIT_TIME;
      },
      isNearby: (context, event, { cond }) => {
        const refMouseEvent: MouseEvent | null = (context as any)[(cond as any).to];
        if (!refMouseEvent) return false;
        const deltaX = Math.abs(event.mouseEvent.offsetX - refMouseEvent.offsetX);
        const deltaY = Math.abs(event.mouseEvent.offsetY - refMouseEvent.offsetY);
        return (deltaX < PAN_DETECTION_DELTA_X) && (deltaY < PAN_DETECTION_DELTA_Y);
      }
    }
  ...
  init() {
    ...
    this.element.addEventListener('mouseleave', this.binded.onMouseLeave, false);
  ...
  dispose() {
    this.element.removeEventListener('mousedown', this.binded.onMouseDown, false);
    this.element.removeEventListener('mousemove', this.binded.onMouseMove, false);
    this.element.removeEventListener('mouseup', this.binded.onMouseUp, false);
    this.element.removeEventListener('mouseleave', this.binded.onMouseLeave, false);
