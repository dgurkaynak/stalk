...
export function convertFromJaegerTrace(rawTrace: any) {
    ...
    if (!_.isArray(rawTrace_.spans)) throw new Error(`"trace.spans" must be array`);
    if (!_.isObject(rawTrace_.processes)) throw new Error(`"trace.processes" must be object`);

    const spans: Span[] = rawTrace_.spans.map((rawSpan: any) => {
        if (!_.isString(rawSpan.spanID) && rawSpan.spanID.length > 0) throw new Error(`"rawSpan.spanID" must be string`);
        if (!_.isString(rawSpan.traceID) && rawSpan.traceID.length > 0) throw new Error(`"rawSpan.traceID" must be string`);
        if (!_.isNumber(rawSpan.startTime)) throw new Error(`"rawSpan.startTime" must be number`);
        ...
        if (_.isArray(rawSpan.logs)) {
            span.logs = rawSpan.logs.map((rawLog: any) => {
                ...
                rawLog.fields.forEach((field: any) => log.fields[field.key] = field.value);
                return log;
            });
        }
        ...
        if (rawSpan.processID && rawTrace_.processes[rawSpan.processID]) {
            ...
            process.tags.forEach((tag: any) => span.process!.tags[tag.key] = tag.value);
        }
