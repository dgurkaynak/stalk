...
export class ZipkinAPI implements API {
    ...
    async search(query: SearchQuery): Promise<SearchResulList> {
        ...
        if (_.isArray(query.tags) && query.tags.length > 0) {
            queryParams.annotationQuery = query.tags.map((data) => {
                if (_.isString(data)) {
                    return data;
                }
         ...
         */
        if (_.isNumber(query.minDuration)) queryParams.minDuration = String(query.minDuration * 1000);
         ...
         */
        if (_.isNumber(query.maxDuration)) queryParams.maxDuration = String(query.maxDuration * 1000);
        ...
        if (!_.isArray(response)) throw new Error('Expected zipkin response must be array');
        return {
            query,
